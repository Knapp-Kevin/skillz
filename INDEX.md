...
The direct `azure-sdk-dotnet` plugin slice is **29/29 current-standard companion-complete**. The direct `azure-sdk-java` plugin has an exact **26-package** denominator and is **13/26 current-standard companion-complete**. The first seven `azure-ai-*` packages are followed by App Configuration and five Azure Communication packages. App Configuration, Call Automation, Chat, Communication Common, and SMS are rejected unchanged for package-specific configuration, communication, recording/privacy, credential, identity/PII, messaging, dependency, and cost defects. CallingServer is explicitly deprecated/superseded and retained as reference-only migration evidence; its verification remains decisively rejected for unchanged use. Behavioral validation remains `not-run` unless separate representative evidence is actually recorded.
