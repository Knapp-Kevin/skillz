...
| Persisted third-party exact-version reviews | 291 |
...
AWS is fully reconciled at **72/72**. Microsoft Skills is active at **119/189**, leaving **70** gaps. Direct `.NET`, Java, Python, and Rust slices are complete. Direct TypeScript is **12/25** from the registered-pin tree. The TypeScript plugin README claims 24 and omits the separately present `applicationinsights-web-ts` package, so the 25-package tree controls.

The current TypeScript companion-complete slice runs through `azure-keyvault-secrets-ts`. All twelve reviewed packages are rejected unchanged under current privacy, sensitive-data, secret-custody, durable-state, production-configuration, external-publication, identity/access, security-control, cryptographic-operation, destructive-lifecycle, or authority hard gates while retaining useful adaptation/reference evidence. `azure-keyvault-secrets-ts` specifically retains Key Vault secret lifecycle, recovery, backup/restore, key lifecycle, and cryptographic-operation mechanics, but raw secret output, literal secret material, purge/recovery, backup/export, key mutation, and cryptographic operations lack mandatory vault/resource, secret-purpose, per-operation, secure-custody, destructive-action, recovery, disclosure, retention, and production authorization. Behavioral validation remains `not-run` unless representative evidence is actually recorded.

The next Microsoft TypeScript unit is `azure-microsoft-playwright-testing-ts`.
