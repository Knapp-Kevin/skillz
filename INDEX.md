...
| Persisted third-party exact-version reviews | 177 |
...
Microsoft Skills is the active admitted-source frontier. It currently has **5** current-standard exact-version companion pairs persisted at the registered pin. `applicationinsights-web-ts` is rejected unchanged under the privacy/authorization rule; `skill-creator`, `cloud-solution-architect`, `copilot-sdk`, and nested plugin package `azure-identity-dotnet` are verified by structured static review. The exact Microsoft eligible denominator is still being reconciled across canonical packages, plugin-hosted skills, nested skills and symlink mirrors, so no completion percentage is claimed. Microsoft Azure Skills follows.
