...
| Persisted third-party exact-version reviews | 283 |
...
AWS is fully reconciled at **72/72**. Microsoft Skills is active at **111/189**, leaving **78** gaps. Direct `.NET`, Java, Python, and Rust slices are complete. Direct TypeScript is **4/25** from the registered-pin tree. The TypeScript plugin README claims 24 and omits the separately present `applicationinsights-web-ts` package, so the 25-package tree controls. `applicationinsights-web-ts`, `azure-ai-contentsafety-ts`, `azure-ai-document-intelligence-ts`, and `azure-ai-projects-ts` now have current-standard companions; all four are rejected unchanged under current privacy, secret-custody, or authority hard gates while retaining useful adaptation/reference evidence. Behavioral validation remains `not-run` unless representative evidence is actually recorded.
