...
| Persisted third-party exact-version reviews | 220 |
| AWS historical reviewed denominator | 72 / 72 |
| AWS current-standard companion-complete | 72 / 72 |
| AWS current-standard gaps | 0 |
| Microsoft Skills current-standard companions | 48 / 189 |
| Microsoft Skills current-standard gaps | 141 |
| Microsoft .NET direct-package companions | 29 / 29 |
| Microsoft Java direct-package companions | 15 / 26 |
...
Microsoft Skills is the active admitted-source frontier. Its exact eligible denominator is **189 independently front-mattered `SKILL.md` entry points** at registered pin `32cad4ee689c95c309e61aeefcbc6af356f1e6a7`; **48/189** currently have current-standard provenance + verification companions, leaving **141** gaps. The denominator is derived from the pinned Git topology rather than the upstream README's inconsistent 174/175 headline counts: language SDK plugin entries, real canonical `.github/skills` packages, Azure plugin packages, Deep Wiki, Microsoft 365 Agents Toolkit, and independently front-mattered nested sub-skills are included; duplicate exposure paths and symlink mirrors are de-duplicated, while ordinary workflow/reference `.md` files are not counted as separate skills.

The direct `azure-sdk-dotnet` plugin slice is **29/29 current-standard companion-complete**. The direct `azure-sdk-java` plugin has an exact **26-package** denominator and is **15/26 current-standard companion-complete**. The latest reviewed Java units add Azure Batch and Azure Cosmos DB. Batch is rejected unchanged because normal examples mutate compute pools/jobs/tasks, execute commands, alter schedules/autoscale state, and can use elevated identities without distinct compute-cost, command/privilege, destructive-state, or scheduling gates. Cosmos DB is rejected unchanged because normal examples create/delete databases and containers, mutate/delete records, issue broad queries, consume metered request units, and print user/item data without distinct data-access, mutation, destructive-action, privacy/logging, or cost boundaries; its top-level dependency also uses `LATEST` rather than an exact SDK version. Behavioral validation remains `not-run` unless separate representative evidence is actually recorded.
