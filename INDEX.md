...
| Persisted third-party exact-version reviews | 430 |
...
The Addy Osmani family is pinned at `f63ec56a3cc936408d792956ae583c3c96a825bd`; the exact upstream denominator is **24** first-class `skills/*/SKILL.md` packages, matching historical #27. The latest current-standard pair is `interview-me`, recovered from exact historical evidence in superseded PR #42 and rejected unchanged **13/20** at exact blob `de5e3aff0c497e2d474e329cde447678632e082b`, with behavioral validation `not-run`. Its one-question-at-a-time clarification, hypothesis surfacing, and explicit out-of-scope restatement remain useful adaptation/reference evidence. Unchanged reuse is rejected because the approximate 95% confidence target and forced rejection of ordinary confirmations are too rigid for portable host-agent behavior. Six companion gaps remain and should preferentially reuse compatible historical evidence.
