...
| Persisted third-party exact-version reviews | 179 |
| AWS historical reviewed denominator | 72 / 72 |
| AWS current-standard companion-complete | 72 / 72 |
| AWS current-standard gaps | 0 |
| Microsoft Skills current-standard companions | 7 / denominator reconciliation in progress |
...
Microsoft Skills is the active admitted-source frontier. It currently has **7** current-standard exact-version companion pairs persisted at the registered pin. `applicationinsights-web-ts`, nested `azure-ai-document-intelligence-dotnet`, and nested `azure-ai-agents-persistent-dotnet` are rejected unchanged under the current privacy/authorization and consequential-action rules; `skill-creator`, `cloud-solution-architect`, `copilot-sdk`, and nested `azure-identity-dotnet` are verified by structured static review. `azure-ai-agents-persistent-dotnet` remains useful SDK/reference material, but its primary flow creates and deletes remote agent resources and invokes tool/external-service paths without a distinct affirmative authorization boundary. The exact Microsoft eligible denominator is still being reconciled across canonical packages, plugin-hosted skills, nested skills and symlink mirrors, so no completion percentage is claimed. Microsoft Azure Skills follows.
