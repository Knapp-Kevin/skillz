...
| Persisted third-party exact-version reviews | 289 |
...
AWS is fully reconciled at **72/72**. Microsoft Skills is active at **117/189**, leaving **72** gaps. Direct `.NET`, Java, Python, and Rust slices are complete. Direct TypeScript is **10/25** from the registered-pin tree. The TypeScript plugin README claims 24 and omits the separately present `applicationinsights-web-ts` package, so the 25-package tree controls.

The current TypeScript companion-complete slice runs through `azure-identity-ts`. All ten reviewed packages are rejected unchanged under current privacy, sensitive-data, secret-custody, durable-state, production-configuration, external-publication, identity/access, security-control, or authority hard gates while retaining useful adaptation/reference evidence. `azure-identity-ts` specifically retains managed/workload identity and credential-selection mechanics, but raw bearer-token logging, verbose identity diagnostics, secret/password credential paths, broad fallback chains, user-identity handling, and CORS/security-control mutation prevent unchanged adoption. Behavioral validation remains `not-run` unless representative evidence is actually recorded.

The next Microsoft TypeScript unit is `azure-keyvault-keys-ts`.
