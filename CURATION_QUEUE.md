...
- **326** persisted exact-version third-party verification companions.
...
- Current-standard companions: **154/189**.
- Remaining gaps: **35**.
...
- `azure-resource-visualizer` — **rejected unchanged, 15/20**. Exact package tree `3a892f03908cf1a388ac9d3da91764bef1e9501c`, `SKILL.md` blob `dc60b612a54d125bca4feb92b2bfead2ee757873`, freshness revision `f2ced3b38ac6974989bd0c0de4c3a45fbd49b38d` from 2026-04-22. Preserve its explicit resource-group selection, complete read-only Azure topology inspection, relationship mapping, secret-placeholder discipline, layered Mermaid synthesis, and architecture-document template. Unchanged adoption is rejected because the bundled Azure Resource Graph reference presents `az extension add --name resource-graph` as a prerequisite, installing persistent local tooling without an action-specific authorization checkpoint. Behavioral validation is `not-run`.

The next Microsoft Skills unit must be established from the registered-pin tree after `azure-resource-visualizer`.
...
After `azure-resource-visualizer` is merged and public accounting is verified at **326 reviews / Microsoft Skills 154/189 / 35 gaps**, establish the first genuine remaining Microsoft Skills gap from the registered-pin tree and continue source-by-source.