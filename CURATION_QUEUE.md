...
- **563** persisted exact-version third-party verification companions.
...
- Current provenance/verification companions: **26/55**; gaps **29**.
- Current decisive states: **13 verified / 13 rejected unchanged**; behavioral validation remains `not-run` for all reviewed units.
- **Skill authoring: 4/4 current-standard complete.**
- **Agent orchestration: 17/17 current-standard complete.**
- **Ops-and-setup: 5/11 reviewed.** `risky-changes` is rejected unchanged 14/20 because paid DeepAPI calls and production-data reads lack distinct action authorization; `global-agent-guardrails` is rejected unchanged 15/20 because persistent user-global security-control mutation lacks a configuration authorization boundary; `google-safe-browsing` is rejected unchanged 14/20 because deployment, DNS TXT mutation, and Search Console review submission are directed without action-specific authorization; `anti-sleep` is verified 17/20 because its local LaunchAgent mutation is directly scoped to the user's explicit anti-sleep request and the bundled launcher verifies exact process and active power-assertion state; `agentic-productivity-setup` is verified 18/20 because it explicitly distinguishes explanation, project build, and installation, keeps webhook secrets out of chat, restricts persistence/transmission to aggregates, and requires permission before live Discord delivery testing.
- Historical issue #47 / PR #50 accounting of **44** skills for this same public pin is superseded by direct pinned-tree evidence. No state is inferred for the remaining 29.
...
Continue issue #66 with the David Ondrej frontier now at **26/55 current-standard companion pairs, 29 genuine gaps**. Skill authoring and agent orchestration are current-standard complete at **4/4** and **17/17**; ops-and-setup is **5/11** reviewed. Continue that bounded category after checking companions and repository history for genuine gaps, while keeping discovery/source-vetting and omission detection parallel and secondary.