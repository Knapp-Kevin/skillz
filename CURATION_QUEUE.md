...
- **180** persisted exact-version third-party verification companions.
...
- Current-standard provenance + verification companions persisted: **8**.
...
- The bounded `.github/plugins/azure-sdk-dotnet/skills/` slice contains **29 direct skill-package directories** at the registered pin. Four now have current-standard companions (`azure-identity-dotnet`, `azure-ai-document-intelligence-dotnet`, `azure-ai-agents-persistent-dotnet`, `azure-ai-openai-dotnet`); the slice still requires full alias/relationship reconciliation before it is folded into a family-wide denominator.
...
**Active frontier:** Microsoft Skills, now with **8** current-standard companion pairs persisted while exact denominator reconciliation continues. Microsoft Azure Skills follows. Discovery and issue/PR hygiene continue in parallel.