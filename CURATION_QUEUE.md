...
- **591** persisted exact-version third-party verification companions.
...
- Current provenance/verification companions: **54/55**; gaps **1**.
- Current decisive states: **21 verified / 33 rejected unchanged**; behavioral validation remains `not-run` for all reviewed units.
...
- **Thinking-and-docs: 12/13 reviewed.** `short` is verified **16/20** as an explicit-invocation, read-only response-compression skill. `stop-overthinking` is verified **15/20** as an explicit-invocation-only, read-only critical-issue check that surfaces serious concerns, otherwise permits proceeding, and asks for concise practical next steps. Bundled OpenAI policy disables implicit invocation. It has no authority hard-fail defect; limitations are minimal failure handling, no explicit evidence-gathering method for judging whether an issue is critical, and modest differentiation from direct prompting. Behavioral validation is `not-run`.
- Historical issue #47 / PR #50 accounting of **44** skills for this same public pin is superseded by direct pinned-tree evidence. No state is inferred for the remaining 1.
...
Continue issue #66 with the David Ondrej frontier now at **54/55 current-standard companion pairs, 1 genuine gap**. Skill authoring, agent orchestration, ops-and-setup, and research-and-web are current-standard complete at **4/4**, **17/17**, **11/11**, and **10/10**. Continue thinking-and-docs, now **12/13**, after checking current companions and recoverable repository history for the final genuine gap. Corey Haines Marketing Skills remains admitted at **0/50** for selective exact-version review after the higher-priority David frontier.
