...
- **568** persisted exact-version third-party verification companions.
...
Continue issue #66 with the David Ondrej frontier now at **31/55 current-standard companion pairs, 24 genuine gaps**. Skill authoring and agent orchestration are current-standard complete at **4/4** and **17/17**; ops-and-setup is **10/11** reviewed. Finish that bounded category with `setup-help` after checking companions and repository history for a genuine gap, while keeping discovery/source-vetting and omission detection parallel and secondary.