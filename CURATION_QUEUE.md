...
- **239** persisted exact-version third-party verification companions.
...
- Current-standard provenance + verification companions persisted: **67/189**.
- Remaining current-standard gaps: **122**.
...
- Direct `.github/plugins/azure-sdk-python/skills/` slice: exact denominator **40**, **8/40 current-standard companion-complete**.

Previously reconciled Microsoft units include `applicationinsights-web-ts`, `skill-creator`, `cloud-solution-architect`, `copilot-sdk`, the complete direct `.NET` SDK slice, the complete direct Java SDK slice, and the first eight direct Python packages. Their exact individual evidence remains in `registry/skills/microsoft-skills/` and `registry/verification/microsoft-skills/` rather than being flattened into a family-level verdict.
...
The direct Python slice has an exact 40-package denominator. `agent-framework-azure-ai-py`, `azure-ai-contentsafety-py`, `azure-ai-contentunderstanding-py`, `azure-ai-language-conversations-py`, `azure-ai-ml-py`, `azure-ai-projects-py`, `azure-ai-textanalytics-py`, and `azure-ai-transcription-py` are now companion-complete at the registered pin. Each retains differentiated evidence rather than inheriting source-level approval.

`azure-ai-textanalytics-py` is rejected unchanged because ordinary examples send arbitrary text, explicit PII, and healthcare/medication information to an external Azure AI service without a distinct data-classification, privacy, consent, disclosure, or regulated-data authority boundary; examples may print raw detected sensitive entities; and dependency installation is unpinned. `azure-ai-transcription-py` is rejected unchanged because normal examples transmit audio to an external AI service without distinct participant-consent, recording-law, sensitive-audio classification/disclosure, retention, transcript-output, or budget/lifecycle authority boundaries; batch transcription creates remote jobs from externally hosted audio; streaming sends local audio externally; authentication uses a subscription key; and installation is unpinned. Behavioral evidence remains `not-run`.
...
**Active frontier:** Microsoft Skills, now **67/189** current-standard companion-complete with **122** gaps. The direct `.NET` plugin slice is **29/29 complete**, the direct Java plugin slice is **26/26 complete**, and the direct Python plugin slice is **8/40**. Continue source-ordered through Python. Microsoft Azure Skills follows after Microsoft Skills. Discovery and issue/PR hygiene continue in parallel.