...
- **200** persisted exact-version third-party verification companions.
...
- Current-standard provenance + verification companions persisted: **28/189**.
- Remaining current-standard gaps: **161**.
...
- `azure-eventhub-dotnet`: nested plugin package tree `61210008d0cda928fe40b6463925443054d79f7b`; `rejected` unchanged at 14/20. Useful Event Hubs guidance is retained across immediate and buffered producers, production EventProcessorClient receiving, partition routing, checkpointing, ASP.NET Core integration, batching, retries and disposal. Normal workflows transmit application event payloads to a metered external service; buffered producers can send asynchronously after enqueue; the production receiver creates the Blob checkpoint container when absent and writes checkpoints; and processing can remain active indefinitely until cancelled, all without distinct affirmative data-transmission/cost/resource-mutation/background-send authorization boundaries or a mandatory approved-data-handling prerequisite. Credential guidance is comparatively strong through DefaultAzureCredential/managed identity and environment-variable based connection strings. Preserve as adaptation/reference evidence with explicit send/background-send, checkpoint-storage mutation, cost, data-governance and long-running processing authorization gates. Behavioral validation is `not-run`.
- `azure-maps-search-dotnet`: nested plugin package tree `4d9a9dcd4d8143792c0f2a377c156524b901e981`; `rejected` unchanged at 14/20. Useful Azure Maps guidance is retained across geocoding, reverse geocoding, routing, route matrices, route ranges, map rendering, IP geolocation, weather, authentication and rate-limit handling. Normal examples transmit addresses, coordinates, route origins/destinations, IP addresses and related location context to a metered external service without a distinct affirmative data-transmission/cost authorization boundary or mandatory approved location/privacy handling prerequisite. The SAS example requests an account SAS token through ARM without a separate credential-issuance authorization boundary, and the map-tile example writes to the local filesystem. Credential examples otherwise use environment variables, placeholders, DefaultAzureCredential and managed identity. Preserve as adaptation/reference evidence with explicit location/privacy, cost, credential-issuance and local-write authorization gates. Behavioral validation is `not-run`.

The first management-plane `.NET` tranche adds fifteen exact-version companion pairs, all decisively `rejected` unchanged after complete package/reference review rather than name-based inference:
...
All fifteen bind the registered Microsoft pin and exact package-tree/SKILL fingerprints. Microsoft’s 2026-05-05 auth-hardening commit is the latest `SKILL.md` revision for every package at that pin. Behavioral validation remains `not-run`. Controlled tag fields use only `registry/taxonomy.yaml`; finer risk distinctions live in permission tiers, dependencies and review notes.
...
- Continue sequential review from the pinned tree and check both companion paths before every unit. The `.github/plugins/azure-sdk-dotnet/skills/` slice contains **29** direct skill-package directories; **24/29** now have current-standard companions. The next bounded tranche is the remaining five direct `.NET` packages.
...
**Active frontier:** Microsoft Skills, now **28/189** current-standard companion-complete with **161** gaps. The direct `.NET` plugin slice is **24/29** complete. Microsoft Azure Skills follows. Discovery and issue/PR hygiene continue in parallel.