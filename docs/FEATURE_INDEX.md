...
| FX13 | Semantic risk audit (negative-rules completeness in high-judgment skills, mutating-action approval language, portable-skill specificity, vendor-API fallback) | `skills/skill-audit/scripts/risk-audit.ts` | `tests/skill-tools.test.mjs::risk-audit-fails-on-mutating-without-approval`, `::risk-audit-fails-on-missing-negative-rules`, `::risk-audit-warns-on-portable-specificity` (WARN ≠ FAIL), `::risk-audit-passes-on-repo`; CI step "Skill risk audit" |
