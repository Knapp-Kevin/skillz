...
| **Persisted third-party review companions** | 108 |
...
- 108 current exact-version third-party verification companion files are persisted.
- AWS Agent Toolkit has an established eligible denominator of 72 skills and a prior full-pass disposition of 70 verified / 2 rejected; 8/72 currently have persisted exact-version provenance + verification companion pairs on `main` (`amazon-bedrock`, `aws-billing-and-cost-management`, `aws-blocks`, `aws-cdk`, `aws-cloudformation`, `aws-containers`, `aws-iam`, and `aws-messaging-and-streaming`). The remaining companion closure is active reconciliation work, not evidence that prior review disappeared.
