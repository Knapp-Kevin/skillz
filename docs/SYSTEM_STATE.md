...
`airunway-aks-setup` and `azure-cloud-migrate` are verified **17/20**. `azure-enterprise-infra-planner` is verified **18/20**. `appinsights-instrumentation`, `azure-ai`, and `azure-aigateway` are rejected unchanged **13/20**. `azure-compliance` is rejected unchanged **12/20**. `azure-compute` is rejected unchanged **11/20**. `azure-cost` is rejected unchanged **14/20**. `azure-deploy` is rejected unchanged **15/20**. `azure-diagnostics` is rejected unchanged **13/20**. `azure-hosted-copilot-sdk` is rejected unchanged **14/20**. Behavioral validation remains `not-run` for all twelve reviewed Azure packages.

`azure-hosted-copilot-sdk` is bound to exact tree `ee166d12b8f9a88a1d4d932a23e844d8203efa13`, `SKILL.md` blob `64b239df00aa320b64a9009c1875b303a87080b1`, and Azure-source freshness revision `4ac35b40b0df5379f3735a4882d3716f998e403d` on 2026-05-08. The package differs from the previously reviewed Microsoft Skills copy, so a fresh exact-version review was required. Preserve its mandatory codebase detection, temp-directory scaffolding safeguard, managed-identity/passwordless authentication guidance, secure GitHub-token/Key Vault flow, BYOM guidance, and explicit prepare/validate/deploy composition. Unchanged adoption fails the current authority hard rule because generic build/modify/prepare triggers lead into a mandatory deployment chain that composes the currently rejected exact-version `azure-deploy` package and therefore exposes consequential post-deployment identity/database mutation paths without a distinct action-specific authorization checkpoint. No literal-secret solicitation into conversational context was identified in the reviewed package; credential guidance favors CLI token acquisition, azd secure environment handling, Key Vault, and managed identities. Behavioral validation remains `not-run`.
