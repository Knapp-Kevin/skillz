...
| **Persisted third-party review companions** | 240 |
...
- **240 exact-version third-party verification companions** are persisted.
...
- Microsoft Skills has an exact eligible denominator of **189** independently front-mattered skill entry points at registered pin `32cad4ee689c95c309e61aeefcbc6af356f1e6a7`; **68/189** have current-standard provenance + verification companions and **121** remain.
...
- The direct `.github/plugins/azure-sdk-python/skills/` slice contains **40** packages and is **9/40 current-standard companion-complete**.
...
Current progress is **68/189** current-standard companion-complete. Previously reconciled units include root/cross-language packages, the complete direct `.NET` SDK plugin slice, the complete direct Java SDK plugin slice, and the first nine direct Python packages.
...
The direct Python plugin has an exact denominator of **40** packages and is **9/40 current-standard companion-complete** at the registered pin. Current exact companions run source-order through `azure-ai-translation-document-py`, including `azure-ai-textanalytics-py` and `azure-ai-transcription-py`. Each package retains differentiated evidence rather than source-level approval.

`azure-ai-translation-document-py` is rejected unchanged because its otherwise useful batch/single-document translation guidance transmits document contents to an external AI service, creates persistent remote jobs, writes translated artifacts to Azure Blob Storage, uses SAS-bearing storage URLs, permits cancellation of active jobs, performs local file reads/writes in the single-document path, and can incur translation/storage cost without distinct data-classification, disclosure, storage-write, job-lifecycle, cancellation, or budget authorization boundaries. Behavioral validation remains `not-run`.
...
1. Continue Microsoft Skills in coherent source-ordered plugin/language batches against the exact **189-entry** denominator; direct Python is the active tranche at **9/40**.
