...
| **Persisted third-party review companions** | 549 |
...
- David Ondrej Skills: exact registered-pin denominator **55**, **12/55** provenance/verification companion complete, gaps **43**; current decisive states **5 verified / 7 rejected unchanged**. Skill authoring is **4/4** complete; agent orchestration is **7/17** reviewed.
...
Current repository evidence contains twelve exact-version companion pairs. Skill authoring is **4/4** current-standard complete. Agent orchestration is **7/17** reviewed: `fable-safe-prompt`, `agent-self-scheduling`, `bb-plugins`, `cmux`, and `codex-subagent` are rejected unchanged, while `bb-subagents` and `corral-launch-agents` are verified. `cmux` exposes consequential send/click/close/settings actions without action-specific authorization; `codex-subagent` launches a workspace-write coding agent without a distinct mutation authorization checkpoint; `corral-launch-agents` explicitly requires a user launch request, dry-run, separate dirty-checkout/adoption authority, secret discipline, and post-launch verification. Together with `goal-loop`, the family is **12/55**, with **43** gaps and **5 verified / 7 rejected unchanged**. Behavioral validation remains `not-run` for all reviewed units.
...
1. Continue corpus-wide provenance audit #66. David Ondrej Skills is now **12/55**, with **43** exact-version companion gaps; skill authoring is complete and agent orchestration is **7/17** reviewed.
