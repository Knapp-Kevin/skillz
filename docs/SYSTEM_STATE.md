...
| **Persisted third-party review companions** | 185 |
| **Pinned external corpora** | 12 |
| **Registered source identities** | 19 |
| **AWS historical review** | 72 / 72 |
| **AWS current-standard companion complete** | 72 / 72 |
| **AWS current-standard gaps** | 0 |
| **Microsoft Skills current-standard companions** | 13 / 189 |
| **Microsoft Skills current-standard gaps** | 176 |
...
- **185 exact-version third-party verification companions** are persisted.
...
- Microsoft Skills has an exact eligible denominator of **189** independently front-mattered skill entry points at registered pin `32cad4ee689c95c309e61aeefcbc6af356f1e6a7`; **13/189** have current-standard provenance + verification companions and **176** remain.
...
Thirteen current-standard Microsoft units are now persisted at the registered pin:
...
- `azure-eventhub-dotnet`, nested under the `azure-sdk-dotnet` plugin, package tree `61210008d0cda928fe40b6463925443054d79f7b`, is `rejected` unchanged by structured static review at 14/20. It provides useful Event Hubs guidance across immediate and buffered producers, production EventProcessorClient receiving, partition routing, checkpointing, ASP.NET Core integration, batching, retries and disposal. Normal workflows transmit application event payloads to a metered external service; buffered producers can send asynchronously after enqueue; the production receiver calls `BlobContainerClient.CreateIfNotExistsAsync()` and writes checkpoints through `UpdateCheckpointAsync()`; and processing can remain active indefinitely until cancelled, all without distinct affirmative data-transmission/cost/resource-mutation/background-send authorization boundaries or a mandatory approved-data-handling prerequisite. Credential guidance is comparatively strong through DefaultAzureCredential/managed identity and environment-variable based connection strings. Preserve as adaptation/reference evidence with explicit send/background-send, checkpoint-storage mutation, cost, data-governance and long-running processing authorization gates. Behavioral validation is `not-run`.
- `azure-maps-search-dotnet`, nested under the `azure-sdk-dotnet` plugin, package tree `4d9a9dcd4d8143792c0f2a377c156524b901e981`, is `rejected` unchanged by structured static review at 14/20. It provides useful Azure Maps guidance across geocoding, reverse geocoding, routing, route matrices, route ranges, map rendering, IP geolocation, weather, authentication and rate-limit handling. Normal examples transmit addresses, coordinates, route origins/destinations, IP addresses and related location context to a metered external service without a distinct affirmative data-transmission/cost authorization boundary or mandatory approved location/privacy handling prerequisite. The SAS example requests an account SAS token through ARM without a separate credential-issuance authorization boundary, and the map-tile example writes to the local filesystem. Credential examples otherwise use environment variables, placeholders, DefaultAzureCredential and managed identity. Preserve as adaptation/reference evidence with explicit location/privacy, cost, credential-issuance and local-write authorization gates. Behavioral validation is `not-run`.
