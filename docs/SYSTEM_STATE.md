...
| **Persisted third-party review companions** | 316 |
...
- Microsoft Skills: exact denominator **189**, **144/189 companion-complete**, **45 gaps** at pin `32cad4ee689c95c309e61aeefcbc6af356f1e6a7`.
...
The non-direct Microsoft plugin frontier is active. Reviewed packages now include `airunway-aks-setup` (15/20), `appinsights-instrumentation` (12/20), `azure-ai` (10/20), `azure-aigateway` (8/20), `azure-cloud-migrate` (9/20), `azure-compliance` (12/20), `azure-compute` (11/20), `azure-cost` (14/20), `azure-deploy` (15/20), `azure-diagnostics` (13/20), `azure-enterprise-infra-planner` (verified, 18/20), and `azure-hosted-copilot-sdk` (**rejected unchanged, 13/20**). The latter is bound to package tree `ee166d12b8f9a88a1d4d932a23e844d8203efa13`, `SKILL.md` blob `64b239df00aa320b64a9009c1875b303a87080b1`, freshness revision `03467cca995f5bd7abc9e2b7ddd7a373543ba08a` from 2026-05-08. Preserve deterministic SDK routing, temp-directory scaffolding, managed identity, least-privilege RBAC, and Key Vault token custody. Unchanged adoption fails because `gh auth refresh --scopes copilot` expands credential authorization without a mandatory action-specific checkpoint and its deployment path inherits the rejected `azure-deploy` authorization gap. Behavioral validation is `not-run`.

Microsoft Skills remains active with **45** gaps. Establish the next gap from the registered-pin tree after `azure-hosted-copilot-sdk`; do not infer it from stale prompts or historical issue text.
...
1. Continue Microsoft Skills in registered-pin source order from the first genuine gap after `azure-hosted-copilot-sdk`.
