...
description: >-
  ...
  skills, and the curated reference corpus, then adversarially validate the
  proposed skill set. Use when the user asks to "bootstrap my skills", "figure
  out what skills I need", "make my agent more consistent", "port how I work
  to another agent", or when onboarding a user who has little or no formal
  skill configuration.
metadata:
...
Turn observed working behavior into a small, portable, evidence-backed skill layer.

This is the front-door orchestration skill for the repository. It does not assume the user already understands skills, the repository layout, or which capabilities should be formalized.
...
- `skill-sync` for supported host deployment.
...
- Never fabricate usage evidence, recurrence, user preferences, test results, token counts, success rates, or host capabilities.
