...
description: >-
  ...
  and GPU/NPU support movements — and synthesize a digest with a GG-CORE
  relevance verdict. Use when the user asks "inference pulse", "what's
  new in llama.cpp", "local inference news", "GGUF changes", or wants
  runtime awareness for GG-CORE work.
metadata:
...
GG-CORE (MythologIQ's local inference runtime) rides these upstreams, so
breaking changes, quantization-format shifts, and GPU/NPU support moves
land here first; every digest ends with a GG-CORE relevance verdict.
...
3. **Synthesize.** Breaking changes first (removed flags, API breaks,
   ...
   then quantization/hardware movements; end with a one-line GG-CORE
   relevance verdict.
...
- Verdicts inform GG-CORE planning; adopting an upstream change stays a
